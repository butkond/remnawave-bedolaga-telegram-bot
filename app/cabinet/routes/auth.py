...
async def _sync_subscription_from_panel_by_email(db: AsyncSession, user: User) -> None:
    """
    Ранее: поиск записей в панели по email и импорт подписок в бота.

    Отключено: в общей панели email/telegram не однозначно идентифицируют «нашего»
    пользователя; связь с панелью только через remnawave UUID, сохранённый в боте.
    """
    return
